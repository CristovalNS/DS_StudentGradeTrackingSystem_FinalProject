...
public class LinkedHashMapBenchmarkTest {
    private static LinkedHashMap<Integer, Classes> classMap = new LinkedHashMap<>();
    private static int classIdCounter = 1;

    static class Classes {
        private String className;
        private LinkedHashMap<String, Student> studentsMap;
        ...
        public LinkedHashMap<String, Student> getStudentsMap() {
            return studentsMap;
        }
...
//        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nBenchmarking performance of LinkedHashMap data structure");
        ...
        LinkedHashMapBenchmarkTest linkedhashMap = new LinkedHashMapBenchmarkTest();
        for (int i = 0; i < n && i < classTracker; i++) {
            linkedhashMap.addClass(className[i]);
        }

        // LinkedHashMap Test Adding Class
//        System.out.println("\nLinkedHashMap: ");
        ...
        // LinkedHashMap Test Removing Class
//        System.out.println("\nLinkedHashMap: ");
        System.out.print("Removing class: ");
        startTime2 = System.nanoTime();
        for (int i = 0; i < n && i < classTracker; i++) {
            linkedhashMap.removeClass(n);
        }
        endTime2 = System.nanoTime();
        getTime(startTime2, endTime2);

        // LinkedHashMap Test Adding Students to Class
...
//        System.out.println("\nLinkedHashMap: ");
        ...
        for (int i = 0; i < o && i < classTracker; i++) {
            for (int x = 0; x < m && x < studentCount; x++) {
                linkedhashMap.addStudent(className[i], studentName[x], studentGrade[x]);
            }
        }
        endTime3 = System.nanoTime();
        getTime(startTime3, endTime3);

        // LinkedHashMap Test Editing Students Grade
        ...
        for (int i = 0; i < o && i < classTracker; i++) {
            for (int x = 0; x < m && x < studentCount; x++) {
                linkedhashMap.setStudentGrade(className[i], studentName[x], String.valueOf(p));
...
//        for (Classes classObj : linkedhashMap.classMap.values()) {
...
//        }
        ...
        // LinkedHashMap Test View Class Details
        ...
        for (int i = 0; i < o && i < classTracker; i++) {
            linkedhashMap.viewClassDetails(className[i]);
        }

        endTime5 = System.nanoTime();
        getTime(startTime5, endTime5);

        // LinkedHashMap Test Removing Students From Class
        ...
        for (int i = 0; i < q && i < classTracker; i++) {
            for (int x = 0; x < r && x < studentCount; x++) {
                linkedhashMap.removeStudent(className[i], studentName[x]);
...
//        }
        ...
        System.out.println("\nThat's the end of LinkedHashMap benchmark! ^-^");
